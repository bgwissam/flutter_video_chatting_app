...
VideoConferencingClient.prototype = {
   ...
   */
  attachVideoConferencingPlugin: function (isRemote, userId, skipMedia, callbacks) {
    var self = this;
    var remoteFeed = null;
      ...
      success: function (pluginHandle) {
        if (isRemote) {
          ...
          var listen = {
            ...
            "ptype": "listener", "feed": userId, "display": displayName
          };
      ...
      onmessage: function (msg, jsep) {
        ...
        if (isRemote) {
          ...
          if (event) {
            // We JOINED
            if (event === "joined") {
              const offerParams = skipMedia ? { useAudio: false, useVideo: false } : { useAudio: true, stream: localStream, useVideo: !self.isOnlyAudio };
              self.createOffer(offerParams, {
                success: function () {
                  ...
                  if (msg["publishers"]) {
                    ...
                    for (var f in publishers) {
                      ...
                      self.emitter.emit(EVENT_PARTICIPANT_JOINED, userId, userDisplayName, true);
                    }
                  }
                },
                error: function (error) {
              ...
              if (msg["publishers"]) {
                ...
                for (var f in publishers) {
                  ...
                  self.emitter.emit(EVENT_PARTICIPANT_JOINED, userId, userDisplayName, false);
                }
